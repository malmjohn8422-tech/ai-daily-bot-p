...
class ScoredSummarizerProcessor(Processor):
    ...
    @staticmethod
    def _output_controls(params: dict) -> dict:
        return {
            "max_recommendations": int(params.get("max_recommendations", 12)),
            "target_words": int(params.get("target_words", 1800)),
            "max_filtered": int(params.get("max_filtered", 12)),
        }

    @staticmethod
    def _quality_issues(content: str, controls: dict) -> list[str]:
        issues = []
        stripped = content.strip()
        if len(stripped) < 400:
            issues.append("内容过短")
        if len(stripped) > controls["target_words"] * 2.8:
            issues.append("内容过长")
        required_markers = ["推荐分", "已过滤", "今日最值得投入时间看的"]
        for marker in required_markers:
            if marker not in content:
                issues.append(f"缺少「{marker}」")
        if "http://" not in content and "https://" not in content:
            issues.append("缺少链接")
        if "原因" not in content and "过滤" in content:
            issues.append("过滤项缺少原因")
        return issues

    def _build_prompt(self, items_text: str, preferences: str, controls: dict, retry_notes: str = "") -> str:
        retry_block = f"\n质量自检反馈：\n{retry_notes}\n请修正以上问题后重新输出。\n" if retry_notes else ""
        return f"""你是一个技术信息雷达编辑。请先根据用户偏好给每条内容打分，再输出一份中文每日简报。

用户偏好：
{preferences}

输出长度：
1. 最多展开 {controls['max_recommendations']} 条推荐内容。
2. 正文目标长度约 {controls['target_words']} 个中文字以内，保持高信息密度，不要写空泛套话。
3. 「已过滤」最多列 {controls['max_filtered']} 条，每条必须包含标题、分数和一句过滤原因。
...
6. 输出主体只展开推荐分达到最低分的内容；未达到最低分的内容放入「已过滤」。
...
{items_text}"""
    ...
    def process(self, data: list[dict], ai_config: dict, params: dict | None = None) -> str:
        ...
        controls = self._output_controls(params)

        prompt = self._build_prompt(items_text, preferences, controls)
        content = call_chat_completion(prompt, ai_config, purpose="个性化日报")
        issues = self._quality_issues(content, controls)
        if issues:
            log.warning(f"个性化日报质量自检未通过，准备重试: {', '.join(issues)}")
            ...
            retry_content = call_chat_completion(retry_prompt, ai_config, purpose="个性化日报质量修正")
            retry_issues = self._quality_issues(retry_content, controls)
            if retry_issues:
                log.warning(f"个性化日报重试后仍有质量问题，将使用重试结果: {', '.join(retry_issues)}")
